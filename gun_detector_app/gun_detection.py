...
# Add any additional imports and functions required for the model (e.g. run_inference, init_crop_region, etc.)
...
if "tflite" in model_name:
    ...
    urllib.request.urlretrieve(model_url, "model.tflite")
...
def detect_keypoints(video_path, video_id):
    cap = cv2.VideoCapture(video_path)
    fps = int(cap.get(cv2.CAP_PROP_FPS))
    video_length = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))

    num_frames_to_process = fps * 1
    num_frames_to_process = min(num_frames_to_process, video_length)
    ...
    image_height, image_width, _ = video_frames[0].shape
    crop_region = init_crop_region(image_height, image_width)

    output_images = []
    for frame_idx in range(num_frames_to_process):
        keypoints_with_scores = run_inference(
            movenet, video_frames[frame_idx], crop_region,
            crop_size=[input_size, input_size])
        output_images.append(draw_prediction_on_image(
            video_frames[frame_idx].astype(np.int32),
    ...
    gif_output_path = f'media/keypoints_gifs/{video_id}_keypoints.gif'
    os.makedirs(os.path.dirname(gif_output_path), exist_ok=True)
    to_gif(output_images, gif_output_path, fps=fps)
